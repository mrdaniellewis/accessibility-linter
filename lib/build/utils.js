...
const toId = exports.toId = docPath => docPath && pathUtils.dirname(toRelative(docPath)).toLowerCase().replace(/[^\w]+/g, '-');
...
(function findTests(dir, depth = 0, docPath) {
  const found = { name: toRelative(dir), depth, docPath };
  ...
  return `
    ...
    module.exports = new Map([
      ${tests.map(test => `[
        ${toSource(test.name)},
        Object.assign(
          { name: ${toSource(test.name)}, doc: ${toSource(toId(test.docPath))} },
