...
it('it does not add an error if the link is hidden', () => {
  const id = uniqueId();
  appendToBody(`<a href="#${id}" style="display: none">link</a>`);
  return whenDomUpdates(() => {
    expect(logger).toNotHaveEntries();
  });
});

it('does not add an error if the link target is hidden', () => {
  const id = uniqueId();
  const el = appendToBody(`<a href="#${id}">link</a><div id="${id}" style="display: none" /><div id="${id}" />`);
  return whenDomUpdates(() => {
    expect(logger).toHaveErrors(['link target is hidden', el]);
  });
});
