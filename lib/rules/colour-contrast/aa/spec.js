...
context('text larger than 18pt', () => {
  ...
  it('adds an error for a contrast of less than 3', () => {
    ...
    return whenDomUpdates(() => {
      expect(logger).toHaveErrors(['contrast is too low 2.96:1', el]);
    });
  });
});
...
context('text larger than 14pt and not bold', () => {
  it('does not add an error for contrast of 3', () => {
    ...
    return whenDomUpdates(() => {
      expect(logger).toHaveErrors(['contrast is too low 3:1', el]);
    });
  });
});
...
context('finding elements', () => {
  ...
  it('adds an for an aria-hidden node', () => {
    ...
    return whenDomUpdates(() => {
      expect(logger).toHaveErrors(['contrast is too low 1:1', el]);
    });
  });
  ...
  it('checks visible siblings following a hidden sibling', () => {
    ...
    return whenDomUpdates(() => {
      expect(logger).toHaveErrors(['contrast is too low 1:1', el.children[1]]);
    });
  });
  ...
  it('does not report errors twice', () => {
    appendToBody('<div><div style="color: #000; background-color: #000"><span>foo</span></div></div>');
    return whenDomUpdates(() => {
      logger.clear();
      linter.run();
      expect(logger).toNotHaveEntries();
