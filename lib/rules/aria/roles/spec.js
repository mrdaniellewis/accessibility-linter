...
context('element is unsupported', () => {
  it('it does not add an error for an implicit role', () => {
    appendToBody('<test-unsupported role="implicit" />');
    return whenDomUpdates(() => {
      expect(logger).toNotHaveEntries();
    });
  });

  it('it does not add an error for an allowed role', () => {
    appendToBody('<test-unsupported role="allowed1" />');
    return whenDomUpdates(() => {
      expect(logger).toNotHaveEntries();
    });
  });

  it('it adds an error for an unknown role', () => {
    const el = appendToBody('<test-unsupported role="unknown" />');
    return whenDomUpdates(() => {
      expect(logger).toHaveErrors(['role "unknown" is not a known role', el]);
    });
  });
