...
context('hidden elements', () => {
  it('reports errors on hidden elements', () => {
    const el = appendToBody('<div aria-string style="display: none;" />');
    return whenDomUpdates(() => {
      expect(logger).toHaveErrors(['aria-string must be a non-empty string', el]);
    });
  });
});
