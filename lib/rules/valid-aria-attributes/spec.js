...
describe('reports invalid integer attributes', () => {
  ...
  it('reports a missing value', async () => {
    ...
    await domChange;
    expect(reporter).toHaveErrors({ message: 'aria-level must be a non-negative integer', element });
  });
  ...
  it('reports a negative number', async () => {
    const element = appendToBody('<div role="heading" aria-level="-1" />');
    await domChange;
    expect(reporter).toHaveErrors({ message: 'aria-level must be a non-negative integer', element });
  });
});
