...
it('adds an error if there is no lang attribute', () => {
  ...
  linter.run();
  expect(logger).toHaveEntries(['missing lang attribute', document.documentElement]);
});

it('adds an error if there is an invalid lang attribute', () => {
  document.documentElement.lang = 'x';
  linter.run();
  expect(logger).toHaveEntries(['language code is invalid', document.documentElement]);
});
