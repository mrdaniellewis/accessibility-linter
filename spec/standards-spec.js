describe('standards', () => {
  const standards = AccessibilityLinter.standards;

  it('is a property of AccessibilityLinter', () => {
    expect(standards).toExist();
  });
  ...
  describe('aria', () => {
    ...
    describe('#aria.roles', () => {
      it('is a list of all possible roles', () => {
        expect(standards.aria.roles).toEqualArray(allRoles);
      });
    });

    describe('#aria.match has the expected return for', () => {
      const match = el => standards.aria.match(el);
      let cleaner;
  ...
  describe('#elements', () => {
    describe('#obsolete', () => {
      it('is a set', () => {
        expect(standards.elements.obsolete).toBeA(Set);
      });
      ...
      obsolete.forEach((name) => {
        it(`it contains ${name}`, () => {
          expect(standards.elements.obsolete.has(name)).toEqual(true);
        });
      });

      it('it contains only obsolete elements', () => {
        expect(standards.elements.obsolete.size).toEqual(obsolete.length);
      });
    });
  });
});
